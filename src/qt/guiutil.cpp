...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("abcchain"))
        return false;
...
{
    // Convert abcchain:// to abcchain:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("abcchain://"))
    {
        uri.replace(0, 12, "abcchain:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ABCChain.lnk";
}
...
{
    return GetAutostartDir() / "abcchain.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ABCChain-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  abcchain-qt [" + tr("command-line options") + "]                     " + "\n";
